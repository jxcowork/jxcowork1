...
//document.write(unescape("%3Cscript src='http://www.teachersun.com/script/ga.js' type='text/javascript'%3E%3C/script%3E"));
</script>
<script type="text/javascript">
try {
...
if id<>"" and cz="sc" and session("tbjb")="radio" then
	...
	conn.execute exec
	conn.close
	response.redirect("bbs.asp?page="+cstr(page))
end if
    ...
    <td><!-- SiteSearch Google -->
      <table border="0" bgcolor="#ffffff">
        <form method="get" action="http://www.google.cn/custom" target="google_window">
          <tr>
            <td nowrap="nowrap" valign="top" align="left" height="32"><a href="http://www.google.com/" title="职称英语、免费网上课堂；孙老师课堂包括：职称考试、孙老师英语、成人三级英语、中小学生英语、中小学生英语考试、英语讲座、大量免费英语讲座"> <img src="http://www.google.com/logos/Logo_25wht.gif" border="0" alt="Google" align="middle"></img></a> </td>
            <td nowrap="nowrap"><input type="hidden" name="domains" value="www.teachersun.com">
              </input>
              ...
              <input type="radio" name="sitesearch" value="www.teachersun.com" checked id="ss1">
              </input>
              <label for="ss1" title="搜索 www.teachersun.com"><font size="-1" color="#000000">www.teachersun.com</font></label>
              <input type="hidden" name="client" value="pub-8617701222786522">
              </input>
              <input type="hidden" name="forid" value="1">
              </input>
              <input type="hidden" name="channel" value="6236647609">
              </input>
...
zts=rs("ooo")
rs.close
exec="select * from Tbl_BBSLT where zid='s' or zid is null order by zd,zhsj desc,id desc"
...
if not  rs.eof then  rs.AbsolutePage =page%>
	...
	hys="1"
          ...
          <td width="54%" height="25">&nbsp;&nbsp;&nbsp; <a href="bbs_tj.asp?ztid=<%=rs("id")%>&ykft=<%=yxft%>&jbm=<%=jbm%>&yc=1&cz=ck&zt=<%=hzt%>" target="_blank" title="<%=rs("zt")%>"> <%=zt%>
            <%if zd="0" then%>
            <font color="#FF0000">[置顶]</font>
            <%end if%>
            </a>
            <%if session("tbjb")="radio" then%>
            &nbsp;&nbsp; <a href="bbs.asp?id=<%=rs("id")%>&cz=sc" onClick="javascript:return confirm('确定删除此贴吗?')"><font color="#FF0000">删除</font></a> &nbsp;|&nbsp;&nbsp;<a href="bbs.asp?id=<%=rs("id")%>&cz=zd&zd=<%=zd%>" onClick="javascript:return confirm('置顶转换吗?')"><font color="#FF0000">置顶转换</font></a>
            <%end if%>
          </td>
...
<%end if%>
      <center>
        <br>
        共<font color="#ff0000"><%=zts%></font>个主题<br>
        &nbsp;&nbsp; 第
      ...
      <%rs.close%>
      <form name="de"  method="post" action="bbs.asp?cz=tj">
        <table width="599" id="tb1">
          <tr>
            <td>标 题</td>
            <td><input type="text" size="50" name="zt" maxlength="200"></td>
          </tr>
          <tr>
            <td>字体颜色</td>
            <td><select name="ys">
          ...
          <tr>
            <td valign="top">内容</td>
            <td><textarea  name="ly" rows="5" cols="60"></textarea></td>
          </tr>
          <tr>
            <td>图片链接</td>
            <td><input type="text" name="tp" size="55" maxlength="100" value="http://"></td>
          </tr>
          <tr>
            <td align="center" colspan="2"><%if session("tbyhm")="" then%>
              <font color="#FF0000">目前您是匿名发贴</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="tb_dl.asp">登录</a>&nbsp;|&nbsp; <a href="tb_zc.asp?jbm=<%=jbm%>">注册</a>
              <%else%>
              <font color="#FF0000"><%=session("tbyhm")%></font>的权限:&nbsp;&nbsp; <a href="tb_zc.asp?cz=xg&jbm=<%=jbm%>">修改信息</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href="bbs.asp?cz=exit">退出</a>
              <%end if%>
            </td>
          </tr>
          <tr>
            <td></td>
            <td><input type="submit" name="Submit" value="提   交" <%if yhm="" and (yxft="0" or yxft=0) then response.write"disabled"%> onClick="javascript:return check(zt.value);">
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" align="center">
  <tr bgcolor="#95ADD2" >
    <td height="25" align="center"><a href='http://www.teachersun.com' target="_blank" title="职称英语、免费网上课堂；孙老师课堂包括：职称考试、孙老师英语、成人三级英语、中小学生英语、中小学生英语考试、英语讲座、大量免费英语讲座">孙老师课堂-贴吧</a></td>
  </tr>
</table>
</body>
</html>
